...
import { Plus, X, Trash2, Clock, CalendarDays } from "lucide-react";
...
export default function CalendarPage() {
  ...
  async function addEvent(e: React.FormEvent) {
    ...
    setTitle(""); setDescription(""); setDate(localToday()); setTime(""); setSaving(false); setShowForm(false);
  }
            ...
            <div key={monthKey} className="space-y-2">
              ...
              <div className="space-y-1.5">
                {monthEvents.map((ev) => (
                  <div
                    key={ev.id}
                    className="group flex items-start gap-3 px-4 py-3 rounded-xl border border-border bg-card hover:border-primary/20 transition-all"
                  >
            ...
            <details className="space-y-2">
              ...
              <div className="space-y-1.5 mt-2">
                {past.slice(-10).reverse().map((ev) => (
                  <div key={ev.id} className="group flex items-start gap-3 px-4 py-3 rounded-xl border border-border bg-card/50 opacity-60">
                    <div className="h-2.5 w-2.5 rounded-full mt-1.5 shrink-0" style={{ backgroundColor: ev.color }} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium line-through">{ev.title}</p>
